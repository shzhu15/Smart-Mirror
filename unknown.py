...
class GUI(tk.Frame):
	def __init__(self, top=None):
		...
		self.News_SectionTitle.place(relx=0.667, rely=0.030, height=31
				, width=304)
		...
		self.News_Stories.configure(wrap='word')
		...
		self.Weather_FeelsLike.configure(text='''Feels Like 25''')
	...
	def create_widgets(self):
		def clock():
			...
			the_time = '' + str(hour % 12) + ':' + str(minutes) + ':' + str(seconds) + ' ' + meridian
			self.Time_Time.configure(text=the_time)
			print(the_time)
			self.Time_Time.after(1000, clock)  # run itself again after 1000 ms

		# create the function that edits the widgets here
		def news():
			api = NewsApiClient(api_key='080248f839564772b9e5d8a39dfae1d5')
			top_headlines = api.get_top_headlines(sources='the-verge')

			self.News_Stories.insert('insert', '-' + top_headlines['articles'][0]['title'] + '\n')
			self.News_Stories.insert('insert' ,'-' + top_headlines['articles'][1]['title'] + '\n')
			self.News_Stories.insert('insert', '-' + top_headlines['articles'][2]['title'] + '\n')
			self.News_Stories.insert('insert', '-' + top_headlines['articles'][3]['title'] + '\n')

			self.News_Stories.after(90000000, news)
		...
		def date():
			...
			self.Time_Date.configure(text=the_date)
			print(the_date)
			self.Time_Time.after(10000000, clock)  # run itself again after 1000 ms
